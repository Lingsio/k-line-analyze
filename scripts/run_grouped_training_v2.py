"""Grouped Training Experiment V2 - Load data once, reuse for all groups"""
...
def create_global_datasets(config):
    """Create datasets once for all groups"""
    common = {
    ...
    print("Loading global datasets (once for all groups)...", flush=True)
    train_ds = StockDataset(mode='train', augment_prob=config['augment_prob'],
    ...
    print(f"Global datasets loaded: train={len(train_ds)}, val={len(val_ds)}, test={len(test_ds)}", flush=True)
    print(f"Stocks in dataset: {sorted(train_ds.data_cache.keys())}", flush=True)
...
def create_group_dataloaders(train_ds, val_ds, test_ds, group_stocks, config):
    """Create dataloaders for a specific stock group using global datasets"""
    # Filter by group stocks
    train_indices = filter_dataset_by_stocks(train_ds, group_stocks)
    val_indices = filter_dataset_by_stocks(val_ds, group_stocks)
    test_indices = filter_dataset_by_stocks(test_ds, group_stocks)
    ...
    return (train_subset, val_subset, test_subset,
            DataLoader(train_subset, shuffle=True, drop_last=True, **loader_kw),
            DataLoader(val_subset, shuffle=False, **loader_kw),
            DataLoader(test_subset, shuffle=False, **loader_kw))
...
def run_single_group(group_name, group_stocks, train_ds_global, val_ds_global, test_ds_global, config, seed):
    ...
    train_ds, val_ds, test_ds, train_loader, val_loader, test_loader = \
        create_group_dataloaders(train_ds_global, val_ds_global, test_ds_global, group_stocks, config)

    print(f"  [{group_name}][seed={seed}] Data: train={len(train_ds)}, val={len(val_ds)}, test={len(test_ds)}", flush=True)

    # Create model
    model = CNNModel(num_classes=2, input_channels=train_ds_global.get_num_channels(),
                     arch='resnet18', pretrained=False).to(DEVICE)

    criterion = nn.CrossEntropyLoss(weight=train_ds_global.get_class_weights().to(DEVICE))
...
def main():
    print("="*80 + f"\nGrouped Training Experiment V2 (US Stocks by Industry)\n" + "="*80, flush=True)
    ...
    train_ds_global, val_ds_global, test_ds_global = create_global_datasets(BASE_CONFIG)
    ...
    for group_id, group_info in stock_groups.items():
        ...
        for i in range(group_config['num_runs']):
            ...
            runs.append(run_single_group(group_id, group_info['stocks'],
                                        train_ds_global, val_ds_global, test_ds_global,
                                        group_config, 42 + i * 100))
    ...
    # Save results
    output = {
        'experiment': 'Grouped Training by Industry (V2)',
        'baseline_f1': 0.5083,
    ...
    output_file = OUTPUT_DIR / f"grouped_training_v2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
